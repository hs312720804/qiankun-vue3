<template>
  <el-form class="modelForm" ref="modelFormEl" :model="modelState.data" :rules="modelState.rules" label-width="120px">
    ...
    <el-form-item v-if="isRootDir" label="AppSecret" prop="appSecret">
      <span>{{ modelState.data.appSecret }}<el-button type="primary" text="true" @click="handleCopy(modelState.data.appSecret)">复制</el-button></span>
    ...
    <!-- {{ modelState.roleList }}
    selfAuditLevelVO -- {{modelState.data.selfAuditLevelVO}} -->
    ...
    <el-form-item>
      ...
      <el-button @click="emits('go-back')">取消</el-button>
    </el-form-item>
  </el-form>
</template>
<script setup lang="ts">
  ...
  import { genApiService } from '@/services/common.ts'
  ...
  function saveData (data) {
    ...
    if (isEdit.value) {
      // update
      apiServer('update')({ data }).then(() => emits('upsert-end'))
    } else {
      // add
      apiServer('add')({ data }).then(() => emits('upsert-end'))
    }
  }
